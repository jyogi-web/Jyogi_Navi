...
| 予算制約 | 月額上限 0円 (完全無料枠運用) | Google Cloud / Supabase / TiDB の無料枠を徹底活用 |
...
| 項目 | 採用技術 | 採用理由 | 評価観点 | 備考 |
| --- | --- | --- | --- | --- |
| ホスティング | Google Cloud Run | **Scale to Zero**（使わない時は0円）が可能。DifyのDocker実行に最適。 | 可用性 / コスト |  |
| フロントエンドホスティング | Cloudflare Pages | CDN・高速 |  |  |
| コンテナ | Docker |  | 再現性 / 可搬性 |  |
| IaC | Terraform |  | 構成管理 / 再現性 |  |
| CI/CD | GithubActions |  | 自動化 / 安定性 | lint/ format/ test/ build |
| 監視 | Sentry |  | 可観測性 / アラート精度 |  |
| ログ管理 | **Cloud Logging** | Google Cloud標準機能。ログの保持と検索が容易。 | トレーサビリティ |  |
| CDN | Cloudlfare |  | レイテンシ最適化 |  |
